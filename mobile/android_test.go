...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.nbn", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for nbn issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "nbn-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/token/nbntest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for nbn and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.nbn", "github.com/token/nbn/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "nbn.aar"), "nbn.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "nbn", "nbntest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {

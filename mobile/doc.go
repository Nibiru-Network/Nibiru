...
// Package nbn contains the simplified mobile APIs to nbn.
//
// The scope of this package is *not* to allow writing a custom nbn client
// with pieces plucked from nbn, rather to allow writing native dapps on
...
// from the nbn repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original nbn version.

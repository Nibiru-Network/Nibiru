...
import (
	...
	"github.com/token/accounts/external"
	"github.com/token/accounts/keystore"
	"github.com/token/accounts/scwallet"
	"github.com/token/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of nbn is "nbn". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "nbn", keep that.
	if name == "nbn" || name == "nbn-testnet" {
		name = "nbn"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "nbn" instances.
var isOldnbnResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by nbn 1.4 are used if they exist.
	if warn, isOld := isOldnbnResource[path]; isOld {
		oldpath := ""
		if c.name() == "nbn" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldnbnResourceWarning, "Using deprecated resource file %s, please move this file to the 'nbn' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "nbn-keystore")
		ephemeral = keydir
	}

...
// Package eth implements the nbn protocol.
package eth
...
import (
	...
	"github.com/token/core/state/pruner"
	...
	"github.com/token/eth/protocols/eth"
	"github.com/token/eth/protocols/snap"
...
// Ethereum implements the nbn full node service.
...
// New creates a new nbn object (including the
// initialisation of the common nbn object)
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// Assemble the nbn object
	chainDb, err := stack.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "nbn/db/chaindata/", false)
	if err != nil {
		return nil, err
	}
	...
	log.Info("Initialising nbn protocol", "network", config.NetworkId, "dbversion", dbVer)

	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, nbn %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
...
// APIs return the collection of RPC services the nbn package offers.

...
// PublicnbnAPI provides an API to access nbn related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicnbnAPI struct {
	b Backend
}

// NewPublicnbnAPI creates a new nbn protocol API.
func NewPublicnbnAPI(b Backend) *PublicnbnAPI {
	return &PublicnbnAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicnbnAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicnbnAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicnbnAPI) Syncing() (interface{}, error) {
...
// Sign calculates an nbn ECDSA signature for:
// keccack256("\x19nbn Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19nbn Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid nbn signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and nbn.
...
// PublicBlockChainAPI provides an API to access the nbn blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new nbn blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current nbn chain config.
...
// PublicDebugAPI is the collection of nbn APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of nbn APIs exposed over the private
// debugging endpoint.
...
// Version returns the current nbn protocol version.

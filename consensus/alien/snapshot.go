...
// the genesis block.
func newSnapshot(config *params.AlienConfig, sigcache *lru.ARCCache, hash common.Hash, votes []*Vote, lcrs uint64,db ethdb.Database) *Snapshot {
	...
	snap := &Snapshot{
		...
		PofMiner:     NewPofMinerSnap(config.GenesisTimestamp),
	...
	snap.SystemConfig.LockParameters[sscEnumCndLock] = &LockParameter{
		...
		RlsPeriod:  uint32(signerPledgeLockParamRlsPeriod/config.Period),
		Interval:   uint32(signerPledgeLockParamInterval/config.Period),
	}
	snap.SystemConfig.LockParameters[sscEnumPofLock] = &LockParameter{
		LockPeriod: uint32(pofPledgeLockParamPeriod / config.Period),
		RlsPeriod:  uint32(pofPledgeLockParamRlsPeriod/ config.Period),
		Interval:   uint32(pofPledgeLockParamInterval/ config.Period),
	}
	...
	snap.SystemConfig.Deposit[sscEnumPofReward] = new(big.Int).Set(minPofLockBalance)
	snap.SystemConfig.Deposit[sscEnumPofInspireReward] = new(big.Int).Set(minBandwidthLockBalance)
	...
	snap.SystemConfig.ManagerAddress[sscEnumFlowReport] = managerAddressPofReport
	...
	snap.SystemConfig.Deposit[sscEnumPofServicePeriod] = new(big.Int).Set(posCommitPeriod)

	snap.Coin,_ = NewCoin(common.Hash{},db)
	return snap
}
...
func loadSnapshot(config *params.AlienConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
	...
	if _, ok := snap.SystemConfig.Deposit[sscEnumPofReward]; !ok || 0 > snap.SystemConfig.Deposit[sscEnumPofReward].Cmp(big.NewInt(0)) {
		snap.SystemConfig.Deposit[sscEnumPofReward] = new(big.Int).Set(minPofLockBalance)
	}
	if _, ok := snap.SystemConfig.Deposit[sscEnumPofInspireReward]; !ok || 0 > snap.SystemConfig.Deposit[sscEnumPofInspireReward].Cmp(big.NewInt(0)) {
		snap.SystemConfig.Deposit[sscEnumPofInspireReward] = new(big.Int).Set(minBandwidthLockBalance)
	}
	...
	if _, ok := snap.SystemConfig.LockParameters[sscEnumPofLock]; !ok {
		snap.SystemConfig.LockParameters[sscEnumPofLock] = &LockParameter{
			LockPeriod: uint32(pofPledgeLockParamPeriod / config.Period),
			RlsPeriod:  uint32(pofPledgeLockParamRlsPeriod/config.Period),
			Interval:   uint32(pofPledgeLockParamInterval/config.Period),
	...
	if _, ok := snap.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod]; !ok || 0 > snap.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod].Cmp(big.NewInt(0)) {
		snap.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod] = new(big.Int).Set(pofDefaultBasePrice)
	}
	if _, ok := snap.SystemConfig.Deposit[sscEnumPofServicePeriod]; !ok || 0 > snap.SystemConfig.Deposit[sscEnumPofServicePeriod].Cmp(big.NewInt(0)) {
		snap.SystemConfig.Deposit[sscEnumPofServicePeriod] = new(big.Int).Set(posCommitPeriod)
	}
	nilHash:=common.Hash{}
	if snap.CoinHash !=nilHash{
		snap.Coin,err= NewCoin(snap.CoinHash,db)
		if err!=nil {
			return  nil,err
		}
		err=snap.Coin.Load(db,snap.CoinHash)
		if err!=nil {
			return  nil,err
		}
	}
...
func (s *Snapshot) store(db ethdb.Database) error {
	err := s.Revenue.saveCacheL1(db)
	if err != nil {
		return err
	}
	err = s.PofMiner.store(db, s.Number)
	if err != nil {
		return err
	}
	if s.Coin != nil {
		s.CoinHash, err = s.Coin.Save(db)
		if err != nil {
			return err
		}
	}
...
func (s *Snapshot) copy() *Snapshot {
	cpy := &Snapshot{
		...
		TallyMiner:      make(map[common.Address]*CandidateState),
		PofPledge:       make(map[common.Address]*PofPledgeItem),
		PofHarvest:     new(big.Int).Set(s.PofHarvest),
		Revenue:         s.Revenue.copy(),
		SystemConfig: SystemParameter{
		...
		PosPledge:   make(map[common.Address]*PosPledgeItem),
		TallySigner: make(map[common.Address]uint64),
		InspireHarvest : new(big.Int).Set(s.InspireHarvest),
	}

	if s.Coin != nil {
		cpy.Coin = s.Coin.Copy()
		cpy.CoinHash = cpy.Coin.Root()
	}
	...
	for who, revenue := range s.RevenuePof {
		cpy.RevenuePof[who] = &RevenueParameter{
			RevenueAddress:  revenue.RevenueAddress,
			RevenueContract: revenue.RevenueContract,
			MultiSignature:  revenue.MultiSignature,
		}
	...
	for who, pledge := range s.PofPledge {
		cpy.PofPledge[who] = &PofPledgeItem{
			Manager:pledge.Manager,
			Active:pledge.Active,
			PledgeAmount:pledge.PledgeAmount,
			Bandwidth:pledge.Bandwidth,
			LastBwValid:pledge.LastBwValid,
			PofPrice:pledge.PofPrice,
	...
	if _, ok := cpy.SystemConfig.Deposit[sscEnumPofReward]; !ok || 0 > cpy.SystemConfig.Deposit[sscEnumPofReward].Cmp(big.NewInt(0)) {
		cpy.SystemConfig.Deposit[sscEnumPofReward] = new(big.Int).Set(minPofLockBalance)
	}
	if _, ok := cpy.SystemConfig.Deposit[sscEnumPofInspireReward]; !ok || 0 > cpy.SystemConfig.Deposit[sscEnumPofInspireReward].Cmp(big.NewInt(0)) {
		cpy.SystemConfig.Deposit[sscEnumPofInspireReward] = new(big.Int).Set(minBandwidthLockBalance)
	}
	...
	if _, ok := cpy.SystemConfig.LockParameters[sscEnumPofLock]; !ok {
		cpy.SystemConfig.LockParameters[sscEnumPofLock] = &LockParameter{
			LockPeriod: uint32(30 * 24 * 60 * 60 / cpy.Period),
			RlsPeriod:  uint32(180 * 24 * 60 * 60 / cpy.Period),
			Interval:   uint32(24 * 60 * 60 / cpy.Period),
		}
	...
	if _, ok := cpy.SystemConfig.ManagerAddress[sscEnumFlowReport]; !ok {
		cpy.SystemConfig.ManagerAddress[sscEnumFlowReport] = managerAddressPofReport
	}
	...
	if _, ok := cpy.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod]; !ok || 0 > cpy.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod].Cmp(big.NewInt(0)) {
		cpy.SystemConfig.Deposit[sscEnumPofWithinBasePricePeriod] = new(big.Int).Set(pofDefaultBasePrice)
	}
	if _, ok := cpy.SystemConfig.Deposit[sscEnumPofServicePeriod]; !ok || 0 > cpy.SystemConfig.Deposit[sscEnumPofServicePeriod].Cmp(big.NewInt(0)) {
		cpy.SystemConfig.Deposit[sscEnumPofServicePeriod] = new(big.Int).Set(posCommitPeriod)
	}
...
func (s *Snapshot) apply(headers []*types.Header, db ethdb.Database) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		calsnap2,err2 := snap.calCoinHashVer(headerExtra.CoinDataRoot, header.Number.Uint64(), db)
		if err2 != nil {
			log.Error("calCoinHash apply", "err", err)
		...
		snap.updateExchangeCoin(headerExtra.ExchangeCoin, header.Number.Uint64())
		...
		snap.updatePofHarvest(headerExtra.PofHarvest)
		...
		snap.updateInspireHarvest(headerExtra.InspireHarvest)
		reSnap,err:= snap.posApply(headerExtra,header, db)
		if err!=nil{
			return reSnap,nil
		}
		pofSnap,err:= snap.pofApply(headerExtra,header, db)
		if err!=nil{
			return pofSnap,nil
		}
	}
	...
	snap.Revenue.Number = snap.Number
	snap.Revenue.Hash = snap.Hash
	err := snap.verifyTallyCnt()
	if err != nil {
		return nil, err
	}
	if snap.Coin !=nil{
		snap.CoinHash, err = snap.Coin.Save(db)
		if err != nil {
			return nil, err
		}
	}
...
func (snap *Snapshot) updatePofHarvest(pofHarvest *big.Int) {
	if 0 < pofHarvest.Cmp(big.NewInt(0)) {
		if nil == snap.PofHarvest {
			snap.PofHarvest = new(big.Int).Set(pofHarvest)
		} else {
			snap.PofHarvest = new(big.Int).Add(snap.PofHarvest, pofHarvest)
		}
	}
}
func (snap *Snapshot) updateInspireHarvest(inspireHarvest *big.Int) {
	if 0 < inspireHarvest.Cmp(big.NewInt(0)) {
		if nil == snap.InspireHarvest {
			snap.InspireHarvest = new(big.Int).Set(inspireHarvest)
		} else {
			snap.InspireHarvest = new(big.Int).Add(snap.InspireHarvest, inspireHarvest)
		}
	}
}
...
func (snap *Snapshot) getFlowRewardBlock() uint64 {
	return accumulatePofRewardInterval / snap.config.Period
}
func (snap *Snapshot) getBlockPreDay() uint64 {
	return secondsPerDay / snap.config.Period
}

func (snap *Snapshot) updateFlowReport(flowReport []MinerPofReportRecord, headerNumber *big.Int) {
	snap.PofMiner.updatePofReport(snap.getFlowRewardBlock(), snap.getBlockPreDay(), flowReport, headerNumber)
	snap.updateCoinBalanceCostTotal(flowReport,headerNumber)
}

func (snap *Snapshot) updateCoinBalanceCostTotal(flowReport []MinerPofReportRecord, headerNumber *big.Int) {
	snap.updateCoinBalanceCost(flowReport,headerNumber)
}
...
func (snap *Snapshot) updateDeviceBind(deviceBind []DeviceBindRecord) {
	for _, item := range deviceBind {
		if item.Type == 0 {
			...
			if item.Bind {
				snap.RevenuePof[item.Device] = &RevenueParameter{
					RevenueAddress:  item.Revenue,
					RevenueContract: item.Contract,
					MultiSignature:  item.MultiSign,
				}
			} else {
				delete(snap.RevenuePof, item.Device)
			}
		}
	}
}

func (snap *Snapshot) updateExchangeCoin(exchangeCoin []ExchangeCoinRecord, number uint64) {
	if snap.Coin !=nil{
		for _, item := range exchangeCoin {
			snap.Coin.Add(item.Target,item.Amount)
		}
	}
}
...
func (snap *Snapshot) updateGrantProfit(grantProfit []consensus.GrantProfitRecord, db ethdb.Database, headerHash common.Hash, number uint64) {
	snap.Revenue.updateGrantProfit(grantProfit, db,headerHash,number)
}
...
func (snap *Snapshot) updateFlowMiner(header *types.Header,db ethdb.Database) {
	...
	if 0 == header.Number.Uint64()%blockPerDay && 0 != header.Number.Uint64() {
		snap.PofMiner.updatePofMinerDaily(blockPerDay, header)
...
func (pitem *PledgeItem) copy() *PledgeItem {
	return &PledgeItem{
		...
		BurnRatio:new(big.Int).Set(pitem.BurnRatio),
		BurnAmount:new(big.Int).Set(pitem.BurnAmount),
	}
}
...
func (f *PofMinerReport) copy() *PofMinerReport {
	return &PofMinerReport{
		Target:       f.Target,
		Hash:         f.Hash,
...
func (s *Snapshot) calPayProfit(db ethdb.Database,header *types.Header) ([]consensus.GrantProfitRecord, error) {
	...
	if isPaySignerRewards(number, period) {
		...
		return s.Revenue.RewardLock.calPayProfit(db, playGrantProfit, header)
	}
	if isPayPofRewards(number, period) {
		log.Info("LockProfitSnap cal pay flow profit")
		return s.Revenue.PofLock.calPayProfit(db, playGrantProfit, header)
	}
	if isPayInspireRewards(number, period) {
		log.Info("LockProfitSnap cal pay inspire profit")
		return s.Revenue.PofInspireLock.calPayProfit(db, playGrantProfit, header)
	}
	if isPayPosExit(number, period) {
		log.Info("LockProfitSnap cal pay POS exit amount")
		return s.Revenue.PosExitLock.calPayProfit(db, playGrantProfit, header)
	}
	if isPayPoFExit(number, period) {
		log.Info("LockProfitSnap cal pay POF exit amount")
		return s.Revenue.PofExitLock.calPayProfit(db, playGrantProfit, header)
	}
	return playGrantProfit, nil
}

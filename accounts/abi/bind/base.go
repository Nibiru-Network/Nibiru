...
// valid nbn transaction.
type TransactOpts struct {
	From   common.Address // nbn account to send the transaction from
...
// nbn network. It contains a collection of methods that are used by the
// higher level contract bindings to operate.
type BoundContract struct {
	address    common.Address     // Deployment address of the contract on the nbn blockchain
	abi        abi.ABI            // Reflect based ABI to access the correct nbn methods
...
// DeployContract deploys a contract onto the nbn blockchain and binds the
...
func (c *BoundContract) Call(opts *CallOpts, results *[]interface{}, method string, params ...interface{}) error {
	...
	var (
		msg    = nbn.CallMsg{From: opts.From, To: &c.address, Data: input}
...
func (c *BoundContract) transact(opts *TransactOpts, contract *common.Address, input []byte) (*types.Transaction, error) {
	...
	if gasLimit == 0 {
		...
		msg := nbn.CallMsg{From: opts.From, To: contract, GasPrice: opts.GasPrice, GasTipCap: opts.GasTipCap, GasFeeCap: opts.GasFeeCap, Value: value, Data: input}

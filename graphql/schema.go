...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte nbn address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an nbn account at a particular block.
    type Account {
    ...
    # Transaction is an nbn transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an nbn account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an nbn account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an nbn block by number or by hash. If neither is

...
import (
	...
	"github.com/token/crypto/signify"
	"github.com/token/internal/build"
	"github.com/token/params"
)

var (
	// Files that end up in the nbn*.zip archive.
	nbnArchiveFiles = []string{
		"COPYING",
		executablePath("nbn"),
	}

	// Files that end up in the nbn-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert nbn contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "nbn bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (nbn Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "nbn",
			Description: "nbn CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "nbn private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "nbn account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "nbnstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basenbn = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		nbn     = "nbn-" + basenbn + ext
		alltools = "nbn-alltools-" + basenbn + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(nbn, nbnArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{nbn, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with nbn-release.pub"
...
func doDockerImage(cmdline []string) {
	var (
		upload = flag.String("upload", "", `Where to upload the docker image (usually "nbn/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - nbn/client-go:latest                            - Pushes to the master branch, nbn only
	//  - nbn/client-go:stable                            - Version tag publish on GitHub, nbn only
	//  - nbn/client-go:alltools-latest                   - Pushes to the master branch, nbn & tools
	//  - nbn/client-go:alltools-stable                   - Version tag publish on GitHub, nbn & tools
	//  - nbn/client-go:release-<major>.<minor>           - Version tag publish on GitHub, nbn only
	//  - nbn/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, nbn & tools
	//  - nbn/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, nbn only
	//  - nbn/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, nbn & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "token/nbn")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "nbn-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the nbn sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "nbn-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "nbn"
...
type debMetadata struct {
	...
	// nbn version being built. Note that this
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the nbn binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"nbn":     nbnTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.nbn.nsi", filepath.Join(*workdir, "nbn.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("nbn-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "nbnstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.nbn", "-v", "github.com/token/nbn/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("nbn.aar", filepath.Join(GOBIN, "nbn.aar"))
		os.Rename("nbn-sources.jar", filepath.Join(GOBIN, "nbn-sources.jar"))
		return
	}
	...
	archive := "nbn-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("nbn.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/token/nbn/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "nbn-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "nbn.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "nbn.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "nbnstore/builds")`)

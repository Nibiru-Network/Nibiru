...
// Verify that Client implements the nbn interfaces.
var (
	_ = nbn.ChainReader(&Client{})
	_ = nbn.TransactionReader(&Client{})
	_ = nbn.ChainStateReader(&Client{})
	_ = nbn.ChainSyncReader(&Client{})
	_ = nbn.ContractCaller(&Client{})
	_ = nbn.GasEstimator(&Client{})
	_ = nbn.GasPricer(&Client{})
	_ = nbn.LogFilterer(&Client{})
	_ = nbn.PendingStateReader(&Client{})
	// _ = nbn.PendingStateEventer(&Client{})
	_ = nbn.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new nbn service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == nbn.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != nbn.NotFound {
		t.Fatal("error should be nbn.NotFound")
	}
}

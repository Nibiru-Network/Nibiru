...
import (
	...
	"github.com/token/internal/utesting"
	"github.com/token/p2p"
	"github.com/token/p2p/rlpx"
)
...
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "nbn", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected nbn protocol version")
		}
		return nil
	default:
...
// negotiatenbnProtocol sets the Conn's nbn protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "nbn" {
			continue
		}
...
loop:
	...
	// make sure nbn protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("nbn protocol version must be set in Conn")
	}
	if status == nil {

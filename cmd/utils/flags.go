...
// Package utils contains internal helper functions for nbn commands.
package utils
...
import (
	...
	"github.com/token/common/fdlimit"
	"github.com/token/consensus"
	"github.com/token/consensus/alien"
	"github.com/token/consensus/clique"
	"github.com/token/consensus/ethash"
	"github.com/token/core"
	"github.com/token/core/rawdb"
	"github.com/token/core/vm"
	"github.com/token/crypto"
	"github.com/token/eth"
	"github.com/token/eth/downloader"
	"github.com/token/eth/ethconfig"
	"github.com/token/eth/gasprice"
	"github.com/token/eth/tracers"
	"github.com/token/ethdb"
	"github.com/token/ethstats"
	"github.com/token/graphql"
	"github.com/token/internal/ethapi"
	"github.com/token/internal/flags"
	"github.com/token/les"
	"github.com/token/log"
	"github.com/token/metrics"
	"github.com/token/metrics/exp"
	"github.com/token/metrics/influxdb"
	"github.com/token/miner"
	"github.com/token/node"
	"github.com/token/p2p"
	"github.com/token/p2p/enode"
	"github.com/token/p2p/nat"
	"github.com/token/p2p/netutil"
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(TestnetFlag.Name) {
			// Maintain compatibility with older nbn configurations storing the
...
// for nbn and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `nbn account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an nbn client to the stack.
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Ethereum) {
	if cfg.SyncMode == downloader.LightSync {
		...
		if err != nil {
			Fatalf("Failed to register the nbn service: %v", err)
		}
...
// RegisterEthStatsService configures the nbn Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the nbn Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. nbn account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// nbn --keystore /tmp/mykeystore --lightkdf account new
//

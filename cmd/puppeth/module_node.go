...
// nodeDockerfile is the Dockerfile required to run an nbn node.
var nodeDockerfile = `
FROM nbn/client-go:latest
...
{{end}}
RUN \
  echo 'nbn --cache 512 init /genesis.json' > nbn.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.nbn/keystore/ && cp /signer.json /root/.nbn/keystore/' >> nbn.sh && \{{end}}
	echo $'exec nbn --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> nbn.sh

ENTRYPOINT ["/bin/sh", "nbn.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an nbn node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.nbn{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.nbnhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new nbn node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 nbn --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.nbn"],
		ethashdir:  infos.volumes["/root/.nbnhash"],
		port:       port,

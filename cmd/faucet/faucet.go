...
//
// nbn is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// nbn is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with nbn. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/token/accounts/keystore"
	"github.com/token/cmd/utils"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the nbn protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for nbn and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // nbn address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an nbn light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // nbn protocol stack
	client *ethclient.Client   // Client connection to the nbn chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the nbn light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the nbn service: %w", err)
	}
...
// close terminates the nbn connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the nbn address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
...
// the uniqueness identifier (user id/username), username, avatar URL and nbn address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the nbn address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No nbn address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and nbn address to fund on
// success.
...
// returning the username, avatar URL and nbn address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the nbn address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain nbn address,

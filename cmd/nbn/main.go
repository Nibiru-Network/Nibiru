...
// nbn is the official command-line client for nbn.
package main
...
import (
	...
	"github.com/token/console/prompt"
...
const (
	clientIdentifier = "nbn" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the nbn command line interface")
...
func init() {
	// Initialize the CLI app and start nbn
	app.Action = nbn
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2021 The nbn Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.TestnetFlag.Name):
		log.Info("Starting nbn on testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting nbn in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting nbn on nbn mainnet...")
	}
...
// nbn is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func nbn(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local nbn node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full nbn node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("nbn service not running: %v", err)
		}

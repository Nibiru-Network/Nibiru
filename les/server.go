...
type LesServer struct {
	...
	archiveMode bool // Flag whether the nbn node runs in archive mode.
...
func NewLesServer(node *node.Node, e ethBackend, config *ethconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "nbn/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "nbn light client service")

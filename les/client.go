...
// Package les implements the Light nbn Subprotocol.
package les
...
func New(stack *node.Node, config *ethconfig.Config) (*Lightnbn, error) {
...
func (s *Lightnbn) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *Lightnbn) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *Lightnbn) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the nbn package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *Lightnbn) APIs() []rpc.API {
...
func (s *Lightnbn) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *Lightnbn) BlockChain() *light.LightChain      { return s.blockchain }
func (s *Lightnbn) TxPool() *light.TxPool              { return s.txPool }
func (s *Lightnbn) Engine() consensus.Engine           { return s.engine }
func (s *Lightnbn) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *Lightnbn) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *Lightnbn) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols returns all the currently configured network protocols to start.
func (s *Lightnbn) Protocols() []p2p.Protocol {
...
// light nbn protocol implementation.
func (s *Lightnbn) Start() error {

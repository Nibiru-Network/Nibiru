...
import (
	...
	"github.com/token/les/flowcontrol"
	"github.com/token/les/utils"
	vfc "github.com/token/les/vflux/client"
	vfs "github.com/token/les/vflux/server"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // nbn protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local nbn node is running in archive mode, advertise ourselves we have
...
// participating in the Light nbn sub-protocol.
...
// for us(server) but for other protocols(e.g. nbn) upon the devp2p they
// may be useful.
